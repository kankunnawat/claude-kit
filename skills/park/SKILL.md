...
3. **Task-local durability.** Keep an undeclared `.handoffs/` file uncommitted; verify it is ignored. Follow the project's authorized tracker, commit, and close-out convention. Commit the handoff only when the project declares a tracked resume location. Record its worktree and branch so `git worktree list --porcelain` plus each checkout's declared resume location or `.handoffs/` can rediscover it without memory.
4. **Optional memory pointer.** Only with explicit memory authorization, write `RESUME FROM: <handoff path>` through the runtime's sanctioned memory mechanism. No memory store or authorization: skip the pointer; task-local discovery remains sufficient.
5. **Resume verb.** Tell the user exactly what to type after /clear: `/pickup`, `/ship <plan>`, or the chain's own skill.
6. **Downloads paste file — only if the user will drive the next session by hand or under a different Claude account** (another account may not share task discovery). `~/Downloads/<slug>.md` per house paste rules: one unwrapped line per paragraph, no em dashes.
...
| Handoff on a branch the fresh session's /pickup can't see | Scan registered worktrees and their task-local handoffs; memory is optional |
