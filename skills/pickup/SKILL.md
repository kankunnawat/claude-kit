...
1. **Gather candidates from ALL of these sources** — work often lives in a worktree the cwd scan can't see:
   - any available `RESUME FROM: <path>` memory pointers, read through the sanctioned runtime mechanism; optional, never required
   ...
   Run task-local discovery without a memory pointer or memory store.
...
2. **Pick or ask.**
   ...
   - Multiple distinct in-flight tasks (pointers or fresh handoffs for different branches/worktrees) → ask the user which to resume with the supported question tool, or plain text if unavailable, one option per task labeled by branch + handoff title + date. Never silently pick one.
